library(tidyverse) # dependency for these functions to work, since they're built on ggplot
library(gganimate) # dependency for plotting -- ggplot animations
...
# Edit these directories to local paths
df_plays <- read_csv("data/plays.csv")
df_games <- read_csv("data/games.csv")

plot_play = function(gameId, playId,
                     plays = df_plays,
                     games = df_games,
                     track = df_tracking) {
  ...
  df_track <- track %>% filter(playId == samp_playId & gameId == samp_gameId) %>%
    dplyr::mutate(
      dir_rad = dir * pi / 180,
      v_x = sin(dir_rad) * s,
      v_y = cos(dir_rad) * s,
  ...
  play_ <- plays %>% filter(playId == samp_playId & gameId == samp_gameId)
  game_ <- games %>%
  ...
  if (play_direction_ == "right") {
    ...
    line_of_scrimmage = 120 - play_$absoluteYardlineNumber
    to_go_line = line_of_scrimmage + play_$yardsToGo
  }
  ...
  play_frames <- plot_field() + 
    ...
    geom_segment(
      data = df_track %>% dplyr::filter(club == game_$visitorTeamAbbr),
    ...
    geom_segment(
      data = df_track %>% dplyr::filter(club == game_$homeTeamAbbr),
      mapping = aes(x = x, y = y, xend = x + v_x, yend = y + v_y),
    ...
    # ball
    geom_point(
      data = df_track %>% dplyr::filter(club == "football"),
      mapping = aes(x = x, y = y),
